...
# Ensure UTF-8 output encoding for Windows terminals (Hindi script & symbols)
...
class SafetyRiskEngine:
    ...
    def _setup_rules(self):
        ...
        self.rules = [
            ...
            # Rule 2: Critical_Close AND Buckled -> High
            ctrl.Rule(
                self.proximity['Critical_Close'] & self.seatbelt['Buckled'],
            ...
            # Rule 8: Safe AND Unbuckled -> Moderate
            ctrl.Rule(self.proximity['Safe'] & self.seatbelt['Unbuckled'], self.risk_score['Moderate']),

            # Rule 9: Safe AND High_Anomaly -> High
            ctrl.Rule(self.proximity['Safe'] & self.ml_anomaly['High_Anomaly'], self.risk_score['High']),

            # Rule 10: Safe AND Suspicious -> Moderate
            ctrl.Rule(self.proximity['Safe'] & self.ml_anomaly['Suspicious'], self.risk_score['Moderate']),

            # Rule 11: Warning AND Unbuckled -> High
            ctrl.Rule(self.proximity['Warning'] & self.seatbelt['Unbuckled'], self.risk_score['High']),
        ]
    ...
    def evaluate(self, telemetry_dict: Dict, target_lang: str = "hi") -> Dict:
        ...
        """
        # Extract inputs
        proximity_val = min(max(float(telemetry_dict['proximity_distance']), 0.0), 15.0)
